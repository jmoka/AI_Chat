...
const processPDFFromPath = async () => {
    try {
        ...
        if (pdfFiles.length === 0) {
            console.log('Nenhum arquivo PDF encontrado na pasta. Continuando sem PDFs.');
            return []; // Retorna uma lista vazia se não houver PDFs
        }
...
app.post('/chat', async (req, res) => {
    try {
        ...
        // Junta todos os textos dos PDFs para fornecer contexto, se houver PDFs
        const pdfText = pdfContent.length > 0
            ? pdfContent.map(pdf => `${pdf.fileName}:\n${pdf.content}`).join("\n\n")
            : 'Nenhum conteúdo de PDF disponível.';
        ...
        const chatCompletion = await groq.chat.completions.create({
            ...
            model: "llama3-8b-8192", // Modelo Llama 3 com 8 bilhões de parâmetros
            temperature: 1,
