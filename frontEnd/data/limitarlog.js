...
const logDir = path.resolve(__dirname, "log"); // Diretório de logs
...
export default function limitarlog() {
  ...
  const arquivosJSON = fs
    .readdirSync(logDir, { withFileTypes: true }) // Lê o diretório de logs
  ...
  if (arquivosJSON.length > maxFiles) {
    ...
    const arquivosParaRemover = arquivosJSON.slice(
      0,
      arquivosJSON.length - maxFiles
    ); // Arquivos a remover
    ...
    console.log(
      "📂 Não há arquivos para remover. Total de arquivos:",
      arquivosJSON.length
    );
  }
}
